...
class SyncService {
  ...
  private async checkDBAvailability() {
    try {
      ...
      // Show user notification about offline features being unavailable
      if (typeof window !== 'undefined') {
        console.warn('Offline features are disabled. The app will work in online-only mode.');
      }
    }
  }
  ...
  private async createServerRecord(tableName: TableName, data: any): Promise<void> {
    ...
    // If this was a sale with items, mark items as synced too
    if (tableName === 'sales' && data.items && Array.isArray(data.items)) {
      for (const item of data.items) {
        await db.saleItems.update(item.id, { saleId: result.id, _synced: true });
      }
    }
  }
  ...
  private async updateServerRecord(tableName: TableName, data: any): Promise<void> {
    ...
    const response = await fetch(`/api/sync/${tableName}/${data.id}`, {
      method: 'PUT',
      headers,
      body: JSON.stringify(data)
    });
  ...
  private async pullFromServer(): Promise<void> {
    ...
    for (const tableName of tables) {
      try {
        ...
        const response = await fetch(`/api/sync/${tableName}?sync=true`, { headers });
